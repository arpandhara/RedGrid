...
export const createRequest = async (req, res) => {
    try {
        ...
        // --- Prepare DB operations (Background) ---
...
export const fulfillRequest = async (req, res) => {
    try {
        ...
        if (isRequester) {
            ...
            if (request.acceptedBy && request.acceptedBy.length > 0) {
                // FIX 2: Use Promise.all instead of forEach to ensure stability
                await Promise.all(request.acceptedBy.map(async (entry) => {
