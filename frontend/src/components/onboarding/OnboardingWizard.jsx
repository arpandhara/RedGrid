...
const OnboardingWizard = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (role === "hospital" && formData.licenseFile) {
        ...
        const { data, error: uploadError } = await supabase.storage
          .from("certificates") // We can reuse the same bucket or make a new 'licenses' one
          .upload(fileName, formData.licenseFile);

        if (uploadError)
          throw new Error("License upload failed: " + uploadError.message);
  ...
  const getIllustration = () => {
    if (step === 1)
      return "/SVG/DrawKit Vector Illustrations COVID-19 & Vaccinations (8).svg";
    if (step === 2) return "/undraw_map-dark_g9xq.svg";
    if (step === 3)
      return "/SVG/DrawKit Vector Illustrations COVID-19 & Vaccinations (12).svg";
      ...
      <div className="flex-1 flex flex-col h-full bg-white dark:bg-black relative">
          ...
          <div className="max-w-2xl mx-auto space-y-8 pb-24">
            <div className="space-y-2">
              <h1 className="text-3xl font-bold text-zinc-900 dark:text-white">
                ...
                {step === 3 &&
                  (role === "donor" ? "Medical Profile" : "Professional Info")}
              </h1>
              <p className="text-zinc-500">
                Please fill strictly accurate details.
              </p>
            </div>
                      ...
                      <label className="flex items-start gap-3 p-4 bg-zinc-50 dark:bg-zinc-900 rounded-xl cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
                        ...
                        <div>
                          ...
                          <span className="text-xs text-zinc-500">
                            Check this if you wish to proceed as a recipient
                            with medical proof.
                          </span>
                        </div>
                      </label>
                  ...
                  <>
                    ...
                    <div className="space-y-2 pt-2">
                      <label className="text-xs font-bold uppercase text-zinc-500 ml-1">
                        Upload Legal Proof (License / GST Cert)
                      </label>
                      <div className="relative border-2 border-dashed border-zinc-300 dark:border-zinc-700 rounded-xl p-6 flex flex-col items-center justify-center text-center hover:border-red-500 transition-colors bg-zinc-50 dark:bg-zinc-900/50">
                        ...
                        {/* Make sure to handle 'licenseFile' in your handleChange if not already handled */}
                        <input
                          type="file"
                          name="licenseFile"
                          onChange={handleChange}
                      ...
                      <p className="text-[10px] text-zinc-400 pl-1">
                        Your account will be pending until this document is
                    ...
                    {formData.accountType === "temporary" && (
                      <div className="p-4 bg-orange-50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/20 rounded-xl space-y-3">
                        ...
                        <p className="text-xs text-orange-600/80">
                          Account auto-deletes after this date.
                        </p>
                      </div>
                    )}
                  </>
        ...
        <div className="p-6 bg-white dark:bg-black border-t border-zinc-100 dark:border-zinc-800 flex justify-between items-center">
          <button
            onClick={prevStep}
            disabled={step === 1}
            className="flex items-center gap-2 text-zinc-500 hover:text-black dark:hover:text-white disabled:opacity-0 transition-opacity"
          >
...
const SelectInput = ({ label, options, ...props }) => (
  <div className="relative group">
    ...
    <div className="relative">
      ...
      <ChevronRight
        className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400 rotate-90 pointer-events-none"
        size={18}
      />
    </div>
  </div>
