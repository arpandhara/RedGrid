...
import { supabase } from '../../api/supabase'; // Import the client we just made
import { 
  ArrowRight, ArrowLeft, MapPin, Activity, Building2, 
  Calendar, CheckCircle2, ShieldAlert, Loader2, User, ChevronRight, UploadCloud, LocateFixed 
} from 'lucide-react';
...
const OnboardingWizard = () => {
  ...
  const cleanErrorMessage = (error) => {
    ...
    if (msg.includes("E11000")) return "This email or phone number is already registered.";
    if (msg.includes("validation")) return "Please check your inputs. Some fields are invalid.";
    ...
    try {
      ...
      const payload = {
        ...
        donorData: role === 'donor' ? {
          ...
          communicableDiseaseName: formData.hasCommunicableDisease ? formData.diseaseName : null,
    ...
    <div className="fixed inset-0 z-50 bg-white dark:bg-black flex flex-col lg:flex-row overflow-hidden font-sans">
      ...
      <div className="hidden lg:flex lg:w-5/12 bg-zinc-900 relative flex-col justify-between p-12 border-r border-zinc-800">
         ...
         <div className="relative z-10">
             <div className="flex items-center gap-3 mb-12">
                 <div className="w-10 h-10 bg-red-600 rounded-xl flex items-center justify-center text-white font-bold text-xl">R</div>
      ...
      <div className="flex-1 flex flex-col h-full bg-white dark:bg-black relative">
          ...
          <div className="max-w-2xl mx-auto space-y-8 pb-24">
            ...
            {step === 3 && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                    {role === 'donor' && (
                        <>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                {/* Clean weight input without scrollbars */}
                                <FloatingInput label="Weight (kg)" type="number" name="weight" value={formData.weight} onChange={handleChange} className="[appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
                            </div>
                            ...
                            <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800">
                                <label className="flex items-start gap-3 p-4 bg-zinc-50 dark:bg-zinc-900 rounded-xl cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
                                    ...
                                    />
                                    <div>
                                        <span className="font-bold text-zinc-900 dark:text-white block">I have a major communicable disease</span>
                                        <span className="text-xs text-zinc-500">Check this if you wish to proceed as a recipient with medical proof.</span>
                                    </div>
                                </label>
                                    ...
                                    <div className="mt-4 p-4 border border-red-200 dark:border-red-900/30 bg-red-50 dark:bg-red-900/10 rounded-xl space-y-4 animate-in slide-in-from-top-2">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="relative border-2 border-dashed border-zinc-300 dark:border-zinc-700 rounded-xl p-6 flex flex-col items-center justify-center text-center hover:border-red-500 transition-colors bg-white dark:bg-zinc-900">
                                                ...
                                                <p className="text-sm text-zinc-500">{formData.certificateFile ? formData.certificateFile.name : "Click to upload (PDF/JPG)"}</p>
                                                <input type="file" name="certificateFile" onChange={handleChange} accept=".pdf,.jpg,.jpeg,.png" className="absolute inset-0 opacity-0 cursor-pointer" />
                                            </div>
                                        </div>
                                    </div>
                                )}
                    ...
                    {/* Keep Hospital/Org inputs same as before if needed */}
                    {role !== 'donor' && <p className="text-zinc-500">Enter your professional details...</p>}
                </div>
            )}
          </div>
        </div>
        ...
        <div className="p-6 bg-white dark:bg-black border-t border-zinc-100 dark:border-zinc-800 flex justify-between items-center">
            ...
            <button onClick={step < totalSteps ? nextStep : handleSubmit} disabled={isLoading} className="bg-red-600 hover:bg-red-700 text-white px-8 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-red-500/30 transition-transform active:scale-95">
...
const FloatingInput = ({ label, className, ...props }) => (
...
const SelectInput = ({ label, options, ...props }) => (
    <div className="relative group">
        <label className="absolute left-4 top-1 text-xs font-bold text-zinc-500 uppercase tracking-wider z-10 pointer-events-none group-focus-within:text-red-500">
            {label}
        </label>
